-- =============================================================
-- ShopFloor Processo — Integração é um posto: passa a exigir a
-- TRAVA DE SEQUÊNCIA (posto anterior do fluxo satisfeito p/ o produto),
-- igual ao Lançamento. Integração pode ser 1º (sem anterior → libera),
-- intermediária ou última.
-- Assinatura muda (+2 params) → DROP da versão antiga antes do replace
-- (senão vira OVERLOAD, não substituição).
-- =============================================================

drop function if exists public.sf_integrar(text,text,text,text,text,text,jsonb);
...
create or replace function public.sf_integrar(
  ...
  p_prev_posto            text,     -- posto anterior exigido no fluxo; '' se Integração é o 1º
  p_prev_precisa_aprovado boolean,  -- true = exige aprovado no anterior; false = só registrado
...
begin
  ...
  -- trava de sequência: a Integração é um posto — exige o anterior do fluxo
  -- satisfeito para o PRODUTO (registrado, ou aprovado quando o anterior tem status).
