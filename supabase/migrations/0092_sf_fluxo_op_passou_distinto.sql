-- Fluxo — corrige "1457/1410" no card: `aprovadas`/`registros` contam BIPES (reteste-aprovado soma 2),
-- então passavam da qtd da OP. Adiciona `passou_distinto` = PEÇAS DISTINTAS que passaram no posto
-- (SN cujo ÚLTIMO registro no posto ≠ 'reprovado') → sempre ≤ qtd. Vira a base do card (modo Total),
-- do "concluído" e do % de 1ª (denominador = passou_distinto + reprovados_sem_reteste = total distinto).
-- Recria a função (muda o returns table → precisa DROP antes). Idempotente.
