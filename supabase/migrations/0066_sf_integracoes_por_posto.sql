-- =============================================================
-- Integração por posto (parte 2): uma integração ATIVA por
-- (produto, posto). Sem isso, integrar o mesmo produto num 2º
-- posto de Integração do fluxo era barrado por "produto já
-- integrado" (a checagem era global, ignorava o posto).
-- Backfill: integrações existentes → posto 'Integração'.
-- =============================================================

alter table public.sf_integracoes add column if not exists posto text not null default 'Integração';
alter table public.sf_integracoes alter column posto drop default;

-- índice de lookup passa a ser por (produto, posto)
drop index if exists sf_integracoes_produto;
create index sf_integracoes_produto on public.sf_integracoes (produto_sn_norm, posto) where status = 'ATIVA';

-- ---------- sf_integrar: unicidade do produto POR POSTO (mesma assinatura → replace puro) ----------
...
begin
  ...
  -- produto já integrado NESTE POSTO (ATIVA)?
  select codigo into v_codigo
  from sf_integracoes
  where produto_sn_norm = p_produto_sn_norm and posto = p_posto and status = 'ATIVA'
  limit 1;
  ...
  insert into sf_integracoes (codigo, colaborador, cliente, pmo, op, produto_sn, produto_sn_norm, qtd_placas, posto)
  values (v_codigo, p_colaborador, p_cliente, p_pmo, p_op, p_produto_sn, p_produto_sn_norm,
          coalesce(jsonb_array_length(p_placas), 0), p_posto)
