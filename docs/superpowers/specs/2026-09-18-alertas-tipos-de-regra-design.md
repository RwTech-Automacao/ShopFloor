...
- **Tempo médio por peça:** **todos** os bipes do posto na janela (qualquer status, qualquer PMO; um bipe = um `data_hora` distinto), ordenados por `data_hora`; intervalos entre bipes consecutivos do posto; com filtro de PMO, ficam só os intervalos que **terminam** num bipe das PMOs da regra (assim, num posto misto, as peças da PMO Y feitas entre duas peças da PMO X não viram lentidão da X; sem filtro, o resultado é o mesmo); descarta intervalos > `pausa_max_min`; média dos restantes (segundos). "{n} peças" = bipes das PMOs da regra na janela. Precisa de ao menos `minimo_bipes` intervalos válidos para avaliar. **Abaixo** = média > `limite_tempo_seg` (dispara). **Normalizou** = média ≤ limite (com o mínimo). Como os intervalos maiores que a pausa são descartados, o limite tem que ser **menor** que a pausa (`limite_tempo_seg < pausa_max_min * 60`, check na tabela e na validação da tela) — senão a regra nunca dispararia. Na janela `op`, vale a mesma regra: **todos** os bipes do posto desde o **primeiro bipe daquela OP naquele posto** (um por `data_hora`), intervalos entre bipes consecutivos do posto, e ficam só os que **terminam** num bipe daquela OP (outra OP intercalada no mesmo posto não vira lentidão); pausas descartadas e mínimo de intervalos como acima. O corte no primeiro bipe da OP também é o limite inferior de leitura da janela `op` na taxa de aprovação (sem mudar o resultado).
- **Defeito repetido:** bipes do posto na janela com `lower(status) = 'reprovado'` e código de defeito preenchido, agrupados por código (aparado: `'2040 X'` e `'2040 X '` são o mesmo defeito); cada código com contagem ≥ N é "abaixo" (dispara) para aquele código; um código que tinha ocorrência viva e volta a < N na janela normaliza. Descrição do defeito vem do catálogo (`sf_defeitos`) quando existir.
